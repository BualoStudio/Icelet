...
如果你希望使用小冰岛进行游戏开发，请参阅 [Icelet Wiki](https://github.com/BualoStudio/Icelet/wiki)。
...
小冰岛的设计初衷是好的。但由于 TurboWarp 的局限性，有些功能是无法实现的，这是我们的遗憾。

1. 小冰岛无法完美适配所有屏幕比例，尽管我们可以通过预设一些屏幕比例从而缓解此问题，但其从根本上无法解决。
2. 小冰岛不支持玩家自定义配色方案，因为没有能够修改造型颜色的扩展。
3. 小冰岛不支持 `.json` 文件的导入，导入任何文件都只能是 `.txt` 文件。
4. 小冰岛只能支持2D游戏的开发，并且支持程度有限。
5. 由于 TurboWarp 限制，小冰岛必须等待所有资源全部加载完毕后才能开始执行初始化程序，这需要比其他游戏引擎更长的加载时间。
...
1. **存在的问题**：报告小冰岛在特定情况下触发的问题、漏洞和错误，甚至解决方案。  
